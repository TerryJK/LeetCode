...
// invalidate the least frequently used item before inserting a new item. For the
// purpose of this problem, when there is a tie (i.e., two or more keys that have
...
// 2 cache.get(2);    // returns -1 (not found)
...
// LinkedHashSet. This can keep the order. Every time, one key is referenced, first
// find the current node corresponding to the key, If the following node exist and
// the frequent is larger by one, add key to the keys of the following node, else
// create a new node and add it following the current node. All operations are
